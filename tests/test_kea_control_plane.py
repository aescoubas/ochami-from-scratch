...
def test_helm_kea_config_uses_direct_http_control_sockets() -> None:
    ...
    configmap = (PROJECT_ROOT / "deploy" / "helm" / "templates" / "kea-configmap.yaml").read_text(encoding="utf-8")
...
def test_helm_kea_pod_removes_control_agent_sidecar() -> None:
    ...
    pod = (PROJECT_ROOT / "deploy" / "helm" / "templates" / "kea-pod.yaml").read_text(encoding="utf-8")
