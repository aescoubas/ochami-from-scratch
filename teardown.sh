...
show_help() {
    ...
    echo "  --interface NAME       PXE interface to clean up (method-level option)"
    echo "  --ip IP                PXE IP to remove from interface during cleanup"
    echo "  --cidr N               PXE CIDR prefix used for IP cleanup"
