from __future__ import annotations

from pathlib import Path

import pytest

from ochami.config import (
    DeployConfig,
    DeploymentMethod,
    DiscoveryMethod,
    DnsConflictPolicy,
    TeardownConfig,
    load_config_file,
    merge_deploy_config,
)


def test_deploy_config_defaults() -> None:
    cfg = DeployConfig(method=DeploymentMethod.DOCKER_COMPOSE)

    assert cfg.method == DeploymentMethod.DOCKER_COMPOSE
    assert cfg.pxe_interface == "virbr-pxe"
    assert cfg.pxe_ip == "192.168.100.2"
    assert cfg.pxe_cidr == 24
    assert cfg.mode == "libvirt"
    assert cfg.num_vms == 0
    assert cfg.discovery_method == DiscoveryMethod.STATIC
    assert cfg.dhcp_start == "192.168.100.100"
    assert cfg.dhcp_end == "192.168.100.200"
    assert cfg.dhcp_netmask == "255.255.255.0"
    assert cfg.dhcp_conflict_policy == "fail"
    assert cfg.set_fs_protected_regular is True


def test_deploy_config_rejects_invalid_ip() -> None:
    with pytest.raises(ValueError, match="pxe_ip"):
        DeployConfig(method=DeploymentMethod.MINIKUBE, pxe_ip="not-an-ip")


def test_deploy_config_requires_dhcp_range_pair() -> None:
    with pytest.raises(ValueError, match="dhcp_start and dhcp_end"):
        DeployConfig(method=DeploymentMethod.MINIKUBE, dhcp_start="192.168.1.10", dhcp_end="")


def test_deploy_config_magellan_requires_target() -> None:
    with pytest.raises(ValueError, match="magellan"):
        DeployConfig(
            method=DeploymentMethod.MINIKUBE,
            discovery_method=DiscoveryMethod.MAGELLAN,
        )


def test_deploy_config_rejects_missing_nodes_file(tmp_path: Path) -> None:
    missing = tmp_path / "does-not-exist.csv"

    with pytest.raises(ValueError, match="nodes_file"):
        DeployConfig(method=DeploymentMethod.QUADLETS, nodes_file=missing)


def test_teardown_config_defaults() -> None:
    cfg = TeardownConfig(method=DeploymentMethod.DOCKER_COMPOSE)

    assert cfg.remove_images is False
    assert cfg.vm_name == "virtual-compute-node"
    assert cfg.skip_confirm is False
    assert cfg.pxe_interface == "virbr-pxe"
    assert cfg.pxe_ip == "192.168.100.2"
    assert cfg.pxe_cidr == 24


def test_teardown_config_rejects_invalid_cidr() -> None:
    with pytest.raises(ValueError, match="pxe_cidr"):
        TeardownConfig(method=DeploymentMethod.MINIKUBE, pxe_cidr=40)


# ── Config file loading tests ──────────────────────────────────────


def test_load_config_file_minimal(tmp_path: Path) -> None:
    cfg_file = tmp_path / "openchami.yaml"
    cfg_file.write_text("method: docker-compose\n")

    values = load_config_file(cfg_file)

    assert values["method"] == DeploymentMethod.DOCKER_COMPOSE
    assert len(values) == 1


def test_load_config_file_coerces_enums(tmp_path: Path) -> None:
    cfg_file = tmp_path / "openchami.yaml"
    cfg_file.write_text(
        "method: quadlets\n"
        "mode: hardware\n"
        "discovery_method: magellan\n"
        "dhcp_conflict_policy: auto-kill\n"
    )

    values = load_config_file(cfg_file)

    assert values["method"] == DeploymentMethod.QUADLETS
    assert values["mode"] == "hardware"
    assert values["discovery_method"] == DiscoveryMethod.MAGELLAN
    assert values["dhcp_conflict_policy"] == DnsConflictPolicy.AUTO_KILL


def test_load_config_file_coerces_nodes_file(tmp_path: Path) -> None:
    cfg_file = tmp_path / "openchami.yaml"
    cfg_file.write_text("method: minikube\nnodes_file: /tmp/nodes.csv\n")

    values = load_config_file(cfg_file)

    assert values["nodes_file"] == Path("/tmp/nodes.csv")


def test_load_config_file_empty_returns_empty(tmp_path: Path) -> None:
    cfg_file = tmp_path / "openchami.yaml"
    cfg_file.write_text("")

    values = load_config_file(cfg_file)

    assert values == {}


def test_load_config_file_rejects_non_mapping(tmp_path: Path) -> None:
    cfg_file = tmp_path / "bad.yaml"
    cfg_file.write_text("- item1\n- item2\n")

    with pytest.raises(ValueError, match="YAML mapping"):
        load_config_file(cfg_file)


def test_load_config_file_rejects_unknown_keys(tmp_path: Path) -> None:
    cfg_file = tmp_path / "bad.yaml"
    cfg_file.write_text("method: minikube\nbogus_key: value\n")

    with pytest.raises(ValueError, match="bogus_key"):
        load_config_file(cfg_file)


def test_load_config_file_rejects_missing_file() -> None:
    with pytest.raises(FileNotFoundError):
        load_config_file(Path("/tmp/does-not-exist-openchami.yaml"))


# ── Merge precedence tests ─────────────────────────────────────────


def test_merge_defaults_only() -> None:
    cfg = merge_deploy_config(file_values={}, cli_overrides={"method": DeploymentMethod.MINIKUBE})

    assert cfg.method == DeploymentMethod.MINIKUBE
    assert cfg.pxe_ip == "192.168.100.2"  # dataclass default


def test_merge_file_overrides_defaults() -> None:
    file_values = {
        "method": DeploymentMethod.DOCKER_COMPOSE,
        "pxe_ip": "10.0.0.1",
        "pxe_cidr": 16,
    }

    cfg = merge_deploy_config(file_values=file_values, cli_overrides={})

    assert cfg.pxe_ip == "10.0.0.1"
    assert cfg.pxe_cidr == 16
    assert cfg.pxe_interface == "virbr-pxe"  # still default


def test_merge_cli_overrides_file() -> None:
    file_values = {
        "method": DeploymentMethod.DOCKER_COMPOSE,
        "pxe_ip": "10.0.0.1",
        "pxe_interface": "eth0",
    }
    cli_overrides = {
        "pxe_ip": "172.16.0.1",
    }

    cfg = merge_deploy_config(file_values=file_values, cli_overrides=cli_overrides)

    assert cfg.method == DeploymentMethod.DOCKER_COMPOSE  # from file
    assert cfg.pxe_ip == "172.16.0.1"  # CLI wins over file
    assert cfg.pxe_interface == "eth0"  # from file


def test_merge_cli_method_overrides_file_method() -> None:
    file_values = {"method": DeploymentMethod.DOCKER_COMPOSE}
    cli_overrides = {"method": DeploymentMethod.QUADLETS}

    cfg = merge_deploy_config(file_values=file_values, cli_overrides=cli_overrides)

    assert cfg.method == DeploymentMethod.QUADLETS


def test_merge_validates_final_config() -> None:
    with pytest.raises(ValueError, match="pxe_ip"):
        merge_deploy_config(
            file_values={"method": DeploymentMethod.MINIKUBE},
            cli_overrides={"pxe_ip": "not-valid"},
        )
