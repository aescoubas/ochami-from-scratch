...
def test_deployment_artifacts_are_nix_generated() -> None:
    """Deployment artifacts are generated by Nix (not hand-written) and may be committed."""
    project_root = Path(__file__).resolve().parents[1]
    # Old hand-written templates directory is gone.
...
def test_makefile_has_new_targets() -> None:
    ...
    assert "TEST_NODE_IMAGE ?= nixos" in content
    assert "OPENCHAMI_TEST_NODE_IMAGE=$(TEST_NODE_IMAGE)" in content
    assert "#boot-artifacts-$(TEST_NODE_IMAGE)" in content
